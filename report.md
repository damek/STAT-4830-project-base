...
Large language models are very good at generating solutions but relatively bad at verifying them. The generation-verification gap has been approached in many different ways, including training an LLM or reward model to assess the reward of a given solution (see [https://arxiv.org/abs/2505.03335](https://arxiv.org/abs/2505.03335) among others). Here we take a different approach: train an LLM to improve upon a *strictly verifiable* problem, with the reward signal being shaped by how well the LLM did in previous attempts.

We are heavily inspired by [AlphaEvolve](https://deepmind.google/blog/alphaevolve-a-gemini-powered-coding-agent-for-designing-advanced-algorithms/), which uses LLMs to generate novel solutions to verifiable math problems in an evolutionary approach and achieves some pretty astonishing results. Notably, AlphaEvolve just uses a prompt engineering harness where it takes existing solutions from a bank, has the LLM propose edits, and then evaluates the solution, and then repeats. We differ from AlphaEvolve in that we also want to add test-time training on top of LLMs during the process. Therefore, we are constrained to the set of open-source models that we can train. Our optimization target is then to fine-tune an existing model to "overfit" to a strictly verifiable problem and thus possibly generate a superhuman result.
...
Many scientific and engineering tasks are hard to solve directly but easy to verify once a candidate is proposed. For example, it is relatively trivial to assess how fast a kernel is at producing a specific operation, but very hard to improve upon existing state-of-the-art kernels for specific tasks. If an AI system can reliably improve candidate quality in these settings, this could generalize to broader classes of verifiable discovery problems. In particular, we believe the AlphaEvolve-style agent approach represents a paradigm shift in the way we utilize LLMs: since generative quality is often unstable and hard to use in a practical way, LLMs are constrained in real world application settings. But an approach that focuses on LLMs being the generator and our system serving as a validation mechanism enables the impact to be far broader and target a wide variety of problems that otherwise would be difficult to do in a one-shot or few-shot scenario. When we add RL training on top of this existing harness, we may be able to further unlock much of the specialization power of AI models that are seen in other domains like chess or video games (vis a vis AlphaGo and AlphaStar), but applied into more useful research areas like the sciences and engineering.
...
- Non-overlap: x_i - x_j_2 \ge 2r, \forall i \ne j
